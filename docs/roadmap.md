# Roadmap

- Collect real-world beta feedback from Fitbit, Pixel Watch, Android and Google Health API v4 users.
- Keep setup friction low across Claude Desktop, Cursor, Codex, Hermes and OpenClaw.
- Keep tracking Google Health API v4 release notes as scopes and data types evolve.
- Add real-account fixtures for Pixel Watch, Fitbit and Google first-party sources.
- Add webhook subscriber tools after read-only flows are proven.
- Add optional write tools only behind explicit opt-in and safety gates.
  - Foundation shipped: remote-write gate, nutrition-write scope preset, offline nutrition normalizer, v4 nutrition DataPoint mapper (v4 body TO-VERIFY). log_nutrition tool pending community PR.
